...
const getChartFilesData = () => {
  ...
  const ccaasMapValue = fs.readFileSync(filePath('latest'), { encoding: 'utf8' })
  const ccaaNames = JSON.parse(ccaasMapValue).map(ccaa => ccaa.ccaa)
  ...
  return ccaaNames.map(ccaaName => getDatasetForCcaa(ccaaHashMap, ccaaName))
}

const getDatasetForCcaa = (ccaaHashMap, ccaaName) => {
  const dataset = {}
  for (const [k, v] of ccaaHashMap) {
    const filterOnlyTotals = (v) => v.ccaa === ccaaName
...
export default getChartFilesData
